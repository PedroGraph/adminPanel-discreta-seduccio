...
export const ReturnsStats = ({ onStatusFilter, currentFilter }: ReturnsStatsProps) => {
  ...
  const t = useI18n();
  const tr = t("returns_stats") as any;
  ...
  const stats = [
    { title: tr.total, value: total, status: "all", color: "purple" },
    { title: tr.pending, value: pending, status: "Pendiente", color: "blue" },
    { title: tr.approved, value: approved, status: "Aprobado", color: "green" },
    { title: tr.rejected, value: rejected, status: "Rechazado", color: "red" }
  ];
