...
export const OrdersStats = ({ stats, onStatusFilter, currentFilter, isLoading }: OrdersStatsProps) => {
  const t = useI18n();
  const statLabels = t("orders_stats") as any;
  const statCards = [
    { title: statLabels.total, value: stats.total, status: "all", color: "purple" },
    { title: statLabels.pending, value: stats.pending, status: "pending", color: "blue" },
    { title: statLabels.completed, value: stats.completed, status: "delivered", color: "green" },
    { title: statLabels.cancelled, value: stats.cancelled, status: "cancelled", color: "red" }
  ];
